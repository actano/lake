...
usage = """
    ...
    reads component.json file from stdin and writes generated Manifest.coffee to stdout.
"""
...
convertFileType = (filenames, ext) ->
    ...
    convertExtension = (filename) ->
        ...
        if newExtension is "undefined"
            throw Error "file extension is not registered for file: #{filename}"
...
generateManifest = (component, cwd = ".", cb) ->
    ...
    convertFilePaths = (component) ->
        ...
        # don't remove templates from scripts in this step, because the 'difference' method would not work then
        ...
        # convert filenames, do not use 'auto mapping' for templates, because it's not unique
        # .js and .jade both is converted into .coffee, reverse method is not possible without directory
    ...
    async.waterfall [
        ...
        (component, globbedFiles, cb) ->
            ...
            mainFile = if component.main then convertFileType(path.basename component.main) else ""

            checkExistingFile = (filePath) ->
                # if this script was not called standalone, need to check file with given cwd
            ...
            lookup = (key, defaultValue) ->
                ...
                context = {}
                _(context).extend component, convertedPaths, globbedFiles, globbedFiles, {mainFile}
                if key.indexOf('.') is -1
            ...
            notAssignedTests = _(globbedFiles.all).difference differenceElements

            if notAssignedTests.length > 0
                notAssignedTestFilesMessage = ("// #{test}" for test in notAssignedTests).join "\n"
                messageHeader = [
                    "// WARNING"
                    "//"
            ...
            ###
            todo: the result of the exported function is not coffee, its only javascript
