...
findProjectRoot (err, existingProjectRoot) ->
    if existingProjectRoot?
        if not parsedArgs.force
            console.log 'WARNING: seems like a .lake directory already ' +
                "exists at #{existingProjectRoot}.\n
    ...
    try
        ...
        if err.code is 'EEXIST'
            unless parsedArgs.force
                console.log 'please rm -rf .lake if you want to re-init. ' +
                    'I will not do such a thing for you.'
                process.exit 1
        else
